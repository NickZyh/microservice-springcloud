package com.springcloud.microservicebaseframework.util;
 ...
 * @Author Zyh
 * @Date 2019/7/7 12:20
 * @Description Redis工具类,其中indexdb表示使用了分库处理
 * @Note 常见操作注释
 *   jedis表示jedis pool中的一个连接
 */
@Component
@Slf4j
public class RedisUtils {
    ...
    private static final String COMMAND_EXECUTE_FAILED = "FAILED";

    /**
     * 获取jedispool中的一个连接,分库操作
     * @return
     */
    private Jedis getJedis(int indexdb) {
        Jedis jedis;
        try {
            jedis = jedisPool.getResource();
            jedis.select(indexdb);
        } catch (Exception e) {
            log.error("get jedis failed:" + e.getMessage());
            throw e;
        }
        return jedis;
    }
    ...
    private void closeJedis(Jedis jedis) {
        try {
            ...
            log.error("jedis close failed:" + e.getMessage());
        }
    }
